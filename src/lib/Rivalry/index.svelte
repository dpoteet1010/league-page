<script>
	...
	const setTradeHistory = (p1, p2) => {
		...
		// ✅ DEBUG LOG: Show all transactions
		console.log("All transactions", transactionsInfo.transactions);
		...
		const trades = transactionsInfo.transactions.filter(transaction => {
			...
			// ✅ DEBUG LOG: Check roster mapping
			console.log(`Trade Transaction - Season ${transaction.season}: Roster IDs`, rosterIDOne, rosterIDTwo);
			console.log("Transaction rosters:", transaction.rosters);
		...
		return trades.map(t => {
			// ✅ DEBUG LOG: Show full trade before organizing
			console.log("Reorganizing trade", t);
			...
			if (rosterOneStartLocation > 0) {
				...
				for (const tradeMove of t.moves ?? []) { // ✅ Safe guard
